...
import {
  ...
  useGetWatchlistEvents, useListCases, useListWatchlists, useSearchEntities, useSyncSources
...
function CitedLedgerMark() {
  return <svg viewBox="0 0 40 40" aria-hidden="true" className="h-[24px] w-[24px]">
...
function SourcesPage() {
  ...
  const syncMutation = useSyncSources({
    mutation: {
  ...
  return <div className="fade-in">
    <PageIntro eyebrow="Source registry / public access" title="Know where it came from"><div className="flex max-w-sm flex-col items-end gap-4 text-right text-xs leading-relaxed text-muted-foreground"><span>Official feeds, notices, court indexes, and investigative sources are kept distinct.</span><button type="button" onClick={() => syncMutation.mutate()} disabled={syncMutation.isPending} data-testid="button-sync-sources" className="inline-flex items-center gap-2 border border-primary px-3 py-2 text-[10px] font-bold text-primary transition-colors hover:bg-primary hover:text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50"><RefreshCw size={12} className={syncMutation.isPending ? 'animate-spin' : ''} /> {syncMutation.isPending ? 'Syncing official feeds…' : 'Sync official feeds'}</button></div></PageIntro>
    ...
    {syncMutation.isError && <div className="mb-6 border border-destructive/30 bg-destructive/5 p-4 text-xs text-destructive" data-testid="status-sync-error">The official feed sync could not be completed. Existing cached records remain available.</div>}
    {syncMutation.isSuccess && <div className="mb-6 border border-accent/30 bg-accent/5 p-4 text-xs text-muted-foreground" data-testid="status-sync-success">Official feed sync complete. Existing records were refreshed or preserved with their source citations.</div>}
    {!!sources.length && <div className="grid gap-4 lg:grid-cols-2" data-testid="source-registry">
       {sources.map((source) => <article key={source.id} className="ledger-surface border border-border bg-card p-5 sm:p-6" data-testid={`source-card-${source.id}`}>
        ...
        <p className="mt-4 text-xs leading-relaxed text-muted-foreground">{source.description}</p>
         <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-t hairline pt-4"><span className="font-data text-[10px] uppercase tracking-wider text-muted-foreground">{source.jurisdiction} · {source.format}</span><a href={source.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[11px] font-bold text-accent hover:underline" data-testid={`link-source-registry-${source.id}`}>{source.sourceStatus === 'official' ? 'Official source' : 'Open source'} <ExternalLink size={12} /></a></div>
      </article>)}
    </div>}
     <div className="mt-8 border border-accent/30 bg-accent/5 p-5 text-xs leading-relaxed text-muted-foreground"><span className="font-bold text-foreground">Source boundary.</span> Sanctions, notices, court proceedings, investigations, corporate registry, PEP, and adverse-media context are separate categories. The ICJ handles disputes between states and advisory opinions; the ICC handles individual criminal cases. A designation, allegation, investigation, or warrant is not a conviction.</div>
  </div>;
}
...
function EntityPage() {
  ...
  const justiceSources = (entity?.sources || []).filter((source: any) => ['court_proceeding', 'investigation'].includes(source.category));
    ...
    {entity && <div className="space-y-8">
      ...
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_380px]">
        <section className="border border-border bg-card" data-testid="section-entity-properties"><div className="border-b hairline px-5 py-4"><div className="eyebrow text-accent">Known attributes</div><h2 className="mt-1 text-sm font-bold">What the sources say</h2></div><div className="divide-y divide-border">{propertyEntries.length ? propertyEntries.map(([key, values]) => <div key={key} className="grid gap-2 px-5 py-4 sm:grid-cols-[180px_1fr]"><div className="font-data text-[10px] uppercase tracking-wider text-muted-foreground">{key}</div><div className="space-y-1 text-sm">{(values as string[]).map((value, i) => <div key={i} data-testid={`text-property-${key}-${i}`}>{value}</div>)}</div></div>) : <div className="p-8 text-center text-sm text-muted-foreground" data-testid="status-empty-properties">No structured attributes available.</div>}</div>
         ...
         <section className="border border-border bg-card" data-testid="section-entity-sources"><div className="flex items-center justify-between border-b hairline px-5 py-4"><div><div className="eyebrow text-accent">Source ledger</div><h2 className="mt-1 text-sm font-bold">Primary citations</h2></div><BookOpen size={16} className="text-muted-foreground" /></div><div className="divide-y divide-border">{entity.sources?.length ? entity.sources.map((source: any, i: number) => <a key={`${source.url}-${i}`} href={source.url} target="_blank" rel="noreferrer" data-testid={`link-source-${i}`} className="group block p-5 transition-colors hover:bg-muted/45"><div className="flex gap-3"><ExternalLink size={14} className="mt-0.5 shrink-0 text-accent" /><div><div className="text-xs font-bold leading-relaxed group-hover:text-accent">{source.title}</div><div className="mt-1 flex flex-wrap gap-x-2 gap-y-1 text-[10px] text-muted-foreground"><span>{source.publisher || 'Public source'}</span>{source.jurisdiction && <><span>·</span><span>{source.jurisdiction}</span></>}{source.category && <><span>·</span><span>{String(source.category).replaceAll('_', ' ')}</span></>}{source.sourceStatus && <><span>·</span><span>{String(source.sourceStatus).replaceAll('_', ' ')}</span></>}</div>{(source.legalStatus || source.measureType) && <div className="mt-2 text-[10px] leading-relaxed text-muted-foreground">{source.legalStatus && <span>Legal status: {source.legalStatus}</span>}{source.legalStatus && source.measureType && <span> · </span>}{source.measureType && <span>Measure: {source.measureType}</span>}</div>}</div></div></a>) : <div className="p-8 text-center text-sm text-muted-foreground">No citations attached.</div>}</div></section>
      </div>
       ...
       <section className="border border-border bg-card" data-testid="section-entity-justice">
         <div className="border-b hairline px-5 py-4"><div className="eyebrow text-accent">Legal context</div><h2 className="mt-1 text-sm font-bold">International justice references</h2><p className="mt-2 max-w-3xl text-[10px] leading-relaxed text-muted-foreground">Only explicitly cited proceedings or investigations are related to this record. The index links below are research starting points, not identity matches.</p></div>
         {justiceSources.length ? <div className="divide-y divide-border">{justiceSources.map((source: any, i: number) => <a key={`${source.url}-${i}`} href={source.url} target="_blank" rel="noreferrer" className="group block p-5 hover:bg-muted/45" data-testid={`link-justice-source-${i}`}><div className="flex gap-3"><ExternalLink size={14} className="mt-0.5 shrink-0 text-accent" /><div><div className="text-xs font-bold group-hover:text-accent">{source.title}</div><div className="mt-1 text-[10px] text-muted-foreground">{source.publisher || 'Official source'} · {source.legalStatus || 'Proceeding or investigation'}{source.jurisdiction ? ` · ${source.jurisdiction}` : ''}</div></div></div></a>)}</div> : <div className="grid gap-3 p-5 sm:grid-cols-2"><div className="border border-border bg-muted/20 p-4"><div className="font-data text-[10px] uppercase tracking-wider text-muted-foreground">ICJ · state proceedings</div><a href="https://www.icj-cij.org/cases" target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-accent hover:underline">Browse ICJ cases <ExternalLink size={12} /></a><p className="mt-2 text-[10px] leading-relaxed text-muted-foreground">The ICJ does not prosecute individuals.</p></div><div className="border border-border bg-muted/20 p-4"><div className="font-data text-[10px] uppercase tracking-wider text-muted-foreground">ICC · individual proceedings</div><a href="https://www.icc-cpi.int/cases" target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-accent hover:underline">Browse ICC cases <ExternalLink size={12} /></a><p className="mt-2 text-[10px] leading-relaxed text-muted-foreground">A warrant or allegation is not a conviction.</p></div></div>}
       </section>
...
function NetworkPanel({ graph, depth, setDepth, loading, error, retry }: { graph: any; depth: number; setDepth: (n: number) => void; loading: boolean; error: boolean; retry: () => void }) {
  ...
  return <section className="border border-border bg-card" data-testid="section-entity-network"><div className="flex flex-col justify-between gap-4 border-b hairline px-5 py-4 sm:flex-row sm:items-center"><div><div className="eyebrow text-accent">Relationship graph</div><h2 className="mt-1 text-sm font-bold">Documented connections</h2></div><div className="flex items-center gap-3"><span className="font-data text-[10px] text-muted-foreground">{graph?.nodes?.length || 0} NODES / {graph?.edges?.length || 0} EDGES</span><button type="button" onClick={() => setDepth(depth === 1 ? 2 : 1)} data-testid="button-toggle-network-depth" className="flex items-center gap-2 border border-border px-3 py-2 text-[11px] font-bold transition-colors hover:border-accent hover:text-accent"><ChevronDown size={13} /> Depth {depth}</button></div></div>
    ...
    {error && <div className="p-5"><ErrorBlock message="Network expansion failed." retry={retry} /></div>}
     {graph?.sourceStatus && graph.sourceStatus !== 'available' && <div className="border-b border-primary/20 bg-primary/5 px-5 py-4 text-xs leading-relaxed text-muted-foreground" data-testid="status-network-source"><span className="font-bold text-foreground">Relationship source status:</span> {graph.message || 'Relationship expansion is not currently available.'}</div>}
